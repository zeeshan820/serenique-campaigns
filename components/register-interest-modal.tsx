...
export function RegisterInterestModal({ children }: { children?: React.ReactNode }) {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [scriptLoaded, setScriptLoaded] = useState(false);
  const [formCreated, setFormCreated] = useState(false);
  const [mounted, setMounted] = useState(false);
  const uniqueId = useId();
  const targetId = `hubspot-register-interest-form-container-${uniqueId.replace(/:/g, '')}`;
  ...
  // Create HubSpot form only when modal is opened and script is loaded
  useEffect(() => {
    let retryCount = 0;
    function tryCreateForm() {
      if (isModalOpen && scriptLoaded && !formCreated) {
    ...
    tryCreateForm();
  }, [isModalOpen, scriptLoaded, formCreated, targetId]);
          ...
          <div className={`fixed inset-0 z-[100000] flex items-center justify-center p-4 transition-all duration-300 ${isModalOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}> 
            ...
            > 
              ...
              <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 bg-gradient-to-r from-slate-800 to-slate-700">
                <div>
                  <h2 className="text-xl font-bold text-white">Register Interest</h2>
